...
dump_packet(const char* name, const char* func, apacket* p)
{
    ...
    if (n == 4) {
        ...
        _snprintf(cmd, sizeof cmd, "%08x", command);
    }

    if (p->msg.arg0 < 256U)
        _snprintf(arg0, sizeof arg0, "%d", p->msg.arg0);
    else
        _snprintf(arg0, sizeof arg0, "0x%x", p->msg.arg0);

    if (p->msg.arg1 < 256U)
        _snprintf(arg1, sizeof arg1, "%d", p->msg.arg1);
    else
        _snprintf(arg1, sizeof arg1, "0x%x", p->msg.arg1);
...
read_packet(int  fd, const char* name, apacket** ppacket)
{
    ...
    char  buff[8];
    if (!name) {
        _snprintf(buff, sizeof buff, "fd=%d", fd);
        name = buff;
    }
    while(len > 0) {
        r = adb_read(fd, p, len);
...
static int list_transports_msg(char*  buffer, size_t  bufferlen)
{
    ...
    _snprintf(head, sizeof(head), "%04x", len);
...
void register_socket_transport(int s, const char *serial, int port, int local)
{
    ...
    if (!serial) {
        _snprintf(buff, sizeof buff, "T-%p", t);
        serial = buff;
    }
