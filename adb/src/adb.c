...
alistener listener_list = { &listener_list, &listener_list, };
...
{
  _snprintf(target_str, target_size, "tcp:%d", server_port);
}
...
void connect_device(char* host, char* buffer, int buffer_size)
{
    ...
    if (portstr) {
        if (portstr - host >= sizeof(hostbuf)) {
            _snprintf(buffer, buffer_size, "bad host name %s", host);
            return;
        }
        // zero terminate the host at the point we found the colon
        hostbuf[portstr - host] = 0;
        if (sscanf(portstr + 1, "%d", &port) == 0) {
            _snprintf(buffer, buffer_size, "bad port number %s", portstr);
            return;
        }
    } else {
    ...
    _snprintf(serial, sizeof(serial), "%s:%d", hostbuf, port);
    if (find_transport(serial)) {
        _snprintf(buffer, buffer_size, "already connected to %s", serial);
        return;
    }

    fd = socket_network_client(hostbuf, port, SOCK_STREAM);
    if (fd < 0) {
        _snprintf(buffer, buffer_size, "unable to connect to %s:%d", host, port);
        return;
    }
    ...
    _snprintf(buffer, buffer_size, "connected to %s", serial);
}
...
void connect_emulator(char* port_spec, char* buffer, int buffer_size)
{
    ...
    *port_separator++ = 0;
    ...
    if (candidate_slot < 0) {
        _snprintf(buffer, buffer_size, "Cannot accept more emulators.");
        return;
    }
...
int handle_host_request(char *service, transport_type ttype, char* serial, int reply_fd, asocket *s)
{
    ...
    if (!strcmp(service, "devices")) {
        ...
        _snprintf(buf, sizeof(buf), "OKAY%04x%s",(unsigned)strlen(buffer),buffer);
        D("Wrote device list \n");
    ...
    if (!strncmp(service, "disconnect:", 11)) {
        ...
        if (serial[0] == 0) {
            ...
            if (!strchr(serial, ':')) {
                _snprintf(hostbuf, sizeof(hostbuf) - 1, "%s:5555", serial);
                serial = hostbuf;
            }
            ...
            if (t) {
                ...
                _snprintf(buffer, sizeof(buffer), "No such device %s", serial);
            }
        }
    ...
    if (!strcmp(service, "version")) {
        char version[12];
        _snprintf(version, sizeof version, "%04x", ADB_SERVER_VERSION);
        _snprintf(buf, sizeof buf, "OKAY%04x%s", (unsigned)strlen(version), version);
        writex(reply_fd, buf, strlen(buf));
        return 0;
    }
    ...
    if(!strncmp(service,"get-serialno",strlen("get-serialno"))) {
       ...
       if (transport && transport->serial) {
        ...
        _snprintf(buf, sizeof buf, "OKAY%04x%s",(unsigned)strlen(out),out);
        writex(reply_fd, buf, strlen(buf));
        return 0;
    }
    ...
    if(!strncmp(service,"get-state",strlen("get-state"))) {
        ...
        _snprintf(buf, sizeof buf, "OKAY%04x%s",(unsigned)strlen(state),state);
        writex(reply_fd, buf, strlen(buf));
        return 0;
    }
    return -1;
