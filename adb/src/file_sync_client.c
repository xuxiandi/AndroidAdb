...
#ifdef WIN32
int timeofday(struct timeval *tp, void *tzp)
{
	time_t clock;
	struct tm tm;
	SYSTEMTIME wtm;
                     ...
                     unsigned mtime, mode_t mode, int verifyApk)
{
    ...
    _snprintf(tmp, sizeof(tmp), ",%d", mode);
    r = strlen(tmp);
                     ...
                     const char *name, int isdir)
{
    ...
    ci->dst = ci->src + ssize;
    _snprintf((char*) ci->src, ssize, isdir ? "%s%s/" : "%s%s", spath, name);
    _snprintf((char*) ci->dst, dsize, isdir ? "%s%s/" : "%s%s", dpath, name);
    ...
    while((de = readdir(d))) {
        ...
        if (S_ISDIR(st.st_mode)) {
            ...
            if(!S_ISREG(st.st_mode) && !S_ISLNK(st.st_mode)) {
                ...
                ci->time = (unsigned int )st.st_mtime;
                ci->mode = st.st_mode;
                ci->size = st.st_size;
                ci->next = *filelist;
                *filelist = ci;
...
int do_sync_push(const char *lpath, const char *rpath, int verifyApk)
{
    ...
    if(S_ISDIR(st.st_mode)) {
        ...
        if((mode != 0) && S_ISDIR(mode)) {
            ...
            if(tmp == 0) return 1;
            _snprintf(tmp, tmplen, "%s/%s", rpath, name);
            rpath = tmp;
        }
        BEGIN();
    ...
    if(S_ISREG(mode) || S_ISLNK(mode) || S_ISCHR(mode) || S_ISBLK(mode)) {
        if(stat(lpath, &st) == 0) {
            if(S_ISDIR(st.st_mode)) {
                ...
                char *tmp = (char*)malloc(tmplen);
                if(tmp == 0) return 1;
                _snprintf(tmp, tmplen, "%s/%s", lpath, name);
                lpath = tmp;
            }
        }
        BEGIN();
