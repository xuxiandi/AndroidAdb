...
static int logcat(transport_type transport, char* serial, int argc, char **argv)
{
    ...
    _snprintf(buf, sizeof(buf), "shell:export ANDROID_LOG_TAGS=\"\\%s\" ; exec logcat", quoted_log_tags);
...
top:
    ...
    if(!strcmp(argv[0], "devices")) {
        char *tmp;
        _snprintf(buf, sizeof buf, "host:%s", argv[0]);
        tmp = adb_query(buf);
        if(tmp) {
    ...
    if(!strcmp(argv[0], "forward")) {
        ...
        if (serial) {
            _snprintf(buf, sizeof buf, "host-serial:%s:forward:%s;%s",serial, argv[1], argv[2]);
        } else if (ttype == kTransportUsb) {
            _snprintf(buf, sizeof buf, "host-usb:forward:%s;%s", argv[1], argv[2]);
        } else if (ttype == kTransportLocal) {
            _snprintf(buf, sizeof buf, "host-local:forward:%s;%s", argv[1], argv[2]);
        } else {
            _snprintf(buf, sizeof buf, "host:forward:%s;%s", argv[1], argv[2]);
        }
        if(adb_command(buf)) {
            fprintf(stderr,"error: %s\n", adb_error());
            return 1;
