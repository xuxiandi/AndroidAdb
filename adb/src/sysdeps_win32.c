...
typedef struct FHClassRec_
{
    ...
    int  (*_fh_write)( FH  f, void*  buf, int  len );
...
static int
_fh_file_write( FH  f,  void*  buf, int   len )
{
    DWORD  wrote_bytes;
...
int  adb_open(const char*  path, int  options)
{
    ...
    _snprintf( f->name, sizeof(f->name), "%d(%s)", _fh_to_int(f), path );
    D( "adb_open: '%s' => fd %d\n", path, _fh_to_int(f) );
    return _fh_to_int(f);
}
...
int  adb_write(int  fd, void*  buf, int  len)
{
    FH     f = _fh_from_int(fd);
...
static int
_fh_socket_write( FH  f, void*  buf, int  len )
{
...
int socket_loopback_client(int port, int type)
{
    ...
    _snprintf( f->name, sizeof(f->name), "%d(lo-client:%s%d)", _fh_to_int(f), type != SOCK_STREAM ? "udp:" : "", port );
...
int socket_loopback_server(int port, int type)
{
    ...
    _snprintf( f->name, sizeof(f->name), "%d(lo-server:%s%d)", _fh_to_int(f), type != SOCK_STREAM ? "udp:" : "", port );
...
int socket_network_client(const char *host, int port, int type)
{
    ...
    _snprintf( f->name, sizeof(f->name), "%d(net-client:%s%d)", _fh_to_int(f), type != SOCK_STREAM ? "udp:" : "", port );
...
int socket_inaddr_any_server(int port, int type)
{
    ...
    _snprintf( f->name, sizeof(f->name), "%d(any-server:%s%d)", _fh_to_int(f), type != SOCK_STREAM ? "udp:" : "", port );
...
int  adb_socket_accept(int  serverfd, struct sockaddr*  addr, socklen_t  *addrlen)
{
    ...
    _snprintf( fh->name, sizeof(fh->name), "%d(accept:%s)", _fh_to_int(fh), serverfh->name );
...
static int
bip_buffer_write( BipBuffer  bip, unsigned char* src, int  len )
{
    int  avail, count = 0;
...
static int
bip_buffer_read( BipBuffer  bip, unsigned char*  dst, int  len )
{
    int  avail, count = 0;
...
_fh_socketpair_read( FH  f, void* buf, int  len )
{
    ...
    return bip_buffer_read( bip, (unsigned char*)buf, len );
}

static int
_fh_socketpair_write( FH  f, void*  buf, int  len )
{
    SocketPair  pair = f->fh_pair;
    BipBuffer   bip;
    ...
    return bip_buffer_write( bip, (unsigned char*)buf, len );
}
...
int  adb_socketpair( int  sv[2] )
{
    ...
    _snprintf( fa->name, sizeof(fa->name), "%d(pair:%d)", sv[0], sv[1] );
    _snprintf( fb->name, sizeof(fb->name), "%d(pair:%d)", sv[1], sv[0] );
